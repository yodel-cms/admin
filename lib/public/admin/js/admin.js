...
function loadNewRecord(modelID, parentID, modelWasSelected) {
  ...
  $('#record').load(htmlURL + '?action=new&model=' + modelID + '&parent=' + parentID, function(text, status, req) {
    if(status == 'error') {
      ...
      showAlert('Unable to load new record', 'Sorry, an error occurred loading a new record form. Please refresh the page and try again.')
...
$('#new_record').live('click', function(event) {
  ...
  $('#record').load(htmlURL + '?new=true', function(text, status, req) {
    $('#activity_spinner').fadeOut();
    if(status == 'error') {
      showAlert('Load error', "Sorry, an error occurred loading a new record form. Please refresh the page and try again.");
    } else {
